...
Same as `inStrain profile`, `inStrain compare` requires the user to think about the minimum read-pair ANI that should be considered. It will use the read-pair ANI selected during the `inStrain profile` commands by default, but the user can also access many other min read-pair ANI values using the ANI (see :ref:`Dealing with "mm"` for more information)
